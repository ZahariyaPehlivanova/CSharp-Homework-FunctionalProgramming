...
class StudentsJoined
{
    static void Main(string[] args)
    {
        ...
        var result =
            ...
            join a in studentsSpeciality on student.FacultyNumber equals a.FacultyNumber
            select new { student.FirstName, student.LastName, student.FacultyNumber, a.SpecialtyName };


        var sorted = result.OrderBy(x => x.FirstName);

        foreach (var student in result)
        {
            Console.WriteLine("Student name-->{0} {1} Student faculty number--->{2} Student speciality--->{3}", student.FirstName, student.LastName, student.FacultyNumber, student.SpecialtyName);
        }
    }
}